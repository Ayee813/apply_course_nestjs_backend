...
import { StudentOrmEntity } from 'src/common/infrastructure/database/typeorm/entities/student.orm';
import { UserOrmEntity } from 'src/common/infrastructure/database/typeorm/entities/user.orm';
import { StudentEducationOrmEntity } from 'src/common/infrastructure/database/typeorm/entities/student-education.orm';
import { CourseOrmEntity } from 'src/common/infrastructure/database/typeorm/entities/coures.orm';
import { paginateQueryBuilder } from 'src/common/interceptor/pagination.builder';
import { PaginationResponse } from 'src/common/pagination/pagination.response';
import { AddEducationDto } from './dto/add-profile.dto';
import { ApplyCourseOrmEntity } from 'src/common/infrastructure/database/typeorm/entities/apply-course.orm';
...
export class StudentService {
  ...
  async createStudent(body: CreateStudentDto): Promise<StudentOrmEntity> {
    try {
      const savedStudent =
  ...
  async getAllStudentProfile(
    query: any,
  ): Promise<PaginationResponse<StudentEducationOrmEntity>> {
    const page = Number(query.page) || 1;
    const limit = Number(query.limit) || 10;
