...
import {
  CourseOrmEntity,
  CourseStatus,
} from 'src/common/infrastructure/database/typeorm/entities/coures.orm';
...
export class CourseService {
  ...
  async getAll(query: any): Promise<PaginationResponse<CourseOrmEntity>> {
  ...
  async createCourse(body: CreateCourseDto): Promise<CourseOrmEntity> {
    try {
      const course = this._course.create({
        ...
        price: body.price,
        registration_start_date: new Date(body.register_start_date), // Fixed mapping
        registration_end_date: new Date(body.register_end_date), // Fixed mapping
  ...
  async updateStatus(id: number, body: StatusDTO): Promise<CourseOrmEntity> {
    const course = await this._course.findOne({
      where: { id },
    });
    if (!course) {
    ...
    const response = await this._course.save(course);
    return response;
  }
}
